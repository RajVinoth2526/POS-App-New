...
export class SettingsComponent implements OnInit, OnDestroy  {
  ...
  ngOnInit(): void {
    ...
    const themeSettingsSubscribe = this.systemSettings.themeSettings$.subscribe((data: ThemeSettings) => {
      if(data == null) return;
    ...
    const profileSubscribe = this.systemSettings.profile$.subscribe((data: Profile) => {
      if(data != null) {
  ...
  onProfileSubmit() {
    ...
    if (this.businessForm.valid) {
      if (this.systemSettings.getProfileValue()?.id) {
        // Update the theme settings
        this.spinnerService.show();
        this.systemSettings.updateProfile(parseInt(this.systemSettings.getProfileValue()?.id), profile).subscribe(response => {
          if (response.success) {
        ...
        this.spinnerService.show();
        this.systemSettings.createProfile(profile).subscribe(response => {
          if (response.success) {
            this.spinnerService.hide();
  ...
  onApplyTheme() {
    const theme = this.themeForm.value;
     ...
     if (this.systemSettings.getCurrentThemeSettingsValue()?.id) {
      // Update the theme settings
      this.spinnerService.show();
      this.systemSettings.updateThemeSettings(parseInt(this.systemSettings.getCurrentThemeSettingsValue()?.id), theme).subscribe(response => {
        if (response.success) {
